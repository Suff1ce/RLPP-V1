// End-to-end runner-style parity: exported DataLoader batches + fixed u01 + emulator + RL update
// vs Python_Ver/export_phase3_run_case.py
...
static Eigen::RowVectorXd load_success_row(const std::string& path) {
    Eigen::MatrixXd m = load_csv_matrix(path);
    if (m.rows() != 1) throw std::runtime_error("success must be 1xn: " + path);
    return m.row(0);
}

static std::string read_text_1line(const std::string& path) {
    std::ifstream f(path);
    if (!f) throw std::runtime_error("missing file: " + path);
    std::string line;
...
int main(int argc, char** argv) {
    std::string dir = "d:/RLPP-master/C++_Ver/Phase_3/run_case_decoding01";
    ...
    const std::string model_name = read_text_1line(dir + "/model_name.txt");
    const bool is_manual = (model_name == "decodingModel_manual");
    const std::string decoder_prefix = is_manual ? "" : read_text_1line(dir + "/decoder_prefix.txt");
    ...
    const int episodes = static_cast<int>(std::llround(load_csv_matrix(dir + "/train_params.csv")(0, 0)));

    try {
        for (int ep = 0; ep < episodes; ++ep) {
            const Eigen::MatrixXd inputUnit = load_csv_matrix(dir + "/inputUnit_ep" + std::to_string(ep) + ".csv");
            const Eigen::VectorXi batch_actions = load_csv_vec_i(dir + "/batchActions_ep" + std::to_string(ep) + ".csv");
            const int n = static_cast<int>(inputUnit.cols());
            ...
            rlpp::ApplyNetsPrioriBatch fwd = rlpp::applynets_priori_forward_with_uniforms(
                inputUnit, W1, W2, ep, priori_m, priori_n, u01
            );
            ...
            std::cout << "[ep " << ep << "] forward max_abs_err p=" << p_err << " hidden=" << h_err << " spk=" << s_err
                      << "\n";
            if (p_err > tol || h_err > tol || s_err > tol) throw std::runtime_error("forward parity failed");

            rlpp::EmulatorResult emu;
            if (is_manual) {
                emu = rlpp::emulator_real_manual_exact(fwd.spk_out, batch_actions, indexes, his);
            } else {
                emu = rlpp::emulator_real_trained_nn_exact(fwd.spk_out, batch_actions, indexes, his, decoder_prefix);
            }
            ...
            std::cout << "[ep " << ep << "] emulator max_abs_err success=" << es << " motor_mism=" << mm << "\n";
            if (!std::isfinite(es) || es > tol || mm != 0) throw std::runtime_error("emulator parity failed");
            ...
            const Eigen::RowVectorXd sm_ref = load_csv_matrix(dir + "/smoothed_ep" + std::to_string(ep) + ".csv").row(0);
            const double sm_err = (smoothed - sm_ref).cwiseAbs().maxCoeff();
            std::cout << "[ep " << ep << "] smoothed reward max_abs_err=" << sm_err << "\n";
            if (sm_err > tol) throw std::runtime_error("smoothed reward parity failed");

            Eigen::MatrixXd dW2, dW1;
            rlpp::getgradient_rl_broadcast(smoothed, fwd.p_output, fwd.spk_out, fwd.hidden_unit, inputUnit, W2, n, dW2, dW1);
            ...
            if (dw2_err > tol || dw1_err > tol) throw std::runtime_error("gradient parity failed");

            const double lr = rlpp::learning_rate_rl_real(ep, max_episode);
            W2 = W2 + lr * dW2;
            W1 = W1 + lr * dW1;
            ...
            if (w2_err > tol || w1_err > tol) throw std::runtime_error("weight update parity failed");
        }
    ...
    std::cout << "Runner-style case parity OK\n";
    return 0;
}
