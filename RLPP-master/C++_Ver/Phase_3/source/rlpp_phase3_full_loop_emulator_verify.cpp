// Verify C++ full RLPP loop: applynets_priori(u01) -> emulator_real_manual -> reward -> grads -> update
// vs Python_Ver/export_phase3_full_loop_emulator_case.py
...
int main(int argc, char** argv) {
    std::string dir = "d:/RLPP-master/C++_Ver/Phase_3/full_loop_emulator_case";
    if (argc >= 2) dir = argv[1];
    if (!fs::is_directory(dir)) {
        std::cerr << "Missing case dir. Run: python Python_Ver/export_phase3_full_loop_emulator_case.py\n";
        return 1;
    }
    ...
    try {
        for (int ep = 0; ep < episodes; ++ep) {
            ...
            rlpp::EmulatorResult emu =
                rlpp::emulator_real_manual_exact(fwd.spk_out, batch_actions, indexes, his);
            ...
            if (!std::isfinite(es) || es > tol || mm != 0) {
                throw std::runtime_error("emulator parity failed");
            }
    ...
    std::cout << "Full-loop emulator case parity OK\n";
    return 0;
}
